<template>
  <div class="fault-page">
    ...
    <div class="chart-row">
      ...
      <div class="chart-col" style="flex: 1">
        <div class="page-card chart-card">
          <h4 class="card-title">
            <icon-pie-chart
              style="margin-right: 8px; color: #f53f3f"
            />故障等级占比
          </h4>
    ...
    <div class="page-card" style="margin-top: 16px">
      <h4 class="card-title">
        <icon-file style="margin-right: 8px; color: #165dff" />故障记录
      </h4>
      <div class="search-bar">
        <a-select
          v-model="search.faultType"
          placeholder="故障类型"
          style="width: 150px"
          allow-clear
        >
        ...
        <template #columns>
          <a-table-column title="故障编号" data-index="faultCode" :width="120">
            <template #cell="{ record }">
              <span>{{ record.faultCode }}</span>
              <a-tag
                v-if="isSevereFault(record.faultLevel)"
                color="#F53F3F"
                size="small"
          ...
          <a-table-column title="设备" :width="140">
            <template #cell="{ record }"
              >{{ record.turbineCode || "--" }}
              {{
                record.turbineName ? `(${record.turbineName})` : ""
              }}</template
            >
          </a-table-column>
          <a-table-column
            title="故障类型"
            data-index="faultType"
            :width="120"
          />
            ...
            <template #cell="{ record }">
              <a-space>
                <a-button type="text" size="small" @click="showDetail(record)"
                  ><icon-eye /> 详情</a-button
                >
                <a-button
                  type="text"
                  size="small"
                  status="success"
                  @click="handleFault(record)"
                  v-if="
                    hasPerm('fault:handle') &&
      ...
      <div class="fault-detail" v-if="currentFault">
        <a-descriptions :column="2" bordered size="medium">
          ...
          <a-descriptions-item label="设备"
            >{{ currentFault.turbineCode }}
            {{ currentFault.turbineName }}</a-descriptions-item
          >
          <a-descriptions-item label="故障类型">{{
            currentFault.faultType
          }}</a-descriptions-item>
          <a-descriptions-item label="故障等级">
            <a-tag :color="levelColor(currentFault.faultLevel)">{{
              levelText(currentFault.faultLevel)
            }}</a-tag>
          </a-descriptions-item>
          <a-descriptions-item label="处理状态">
            <a-tag :color="statusColor(currentFault.status)">{{
              statusText(currentFault.status)
            }}</a-tag>
          </a-descriptions-item>
          <a-descriptions-item label="处理人">{{
            currentFault.handler || "--"
          }}</a-descriptions-item>
          <a-descriptions-item label="故障时间" :span="2">{{
            currentFault.faultTime || "--"
          }}</a-descriptions-item>
          <a-descriptions-item label="解决时间" :span="2">{{
            currentFault.resolveTime || "--"
          }}</a-descriptions-item>
          ...
          <a-descriptions-item label="诊断分析" :span="2">
            <div class="diagnosis-content">
              {{ currentFault.diagnosis || "暂无诊断分析" }}
            </div>
          </a-descriptions-item>
          <a-descriptions-item label="处理方案" :span="2">
            <div class="solution-content">
              {{ currentFault.solution || "暂无处理方案" }}
            </div>
          </a-descriptions-item>
        </a-descriptions>
      </div>
...
const hasPerm = (code) =>
  userStore.permissions.length === 0 || userStore.hasPermission(code);
...
const levelText = (l) =>
  ({ LOW: "低", MEDIUM: "中", HIGH: "高", CRITICAL: "严重" })[l] || l;
const levelColor = (l) =>
  ({ LOW: "green", MEDIUM: "orangered", HIGH: "red", CRITICAL: "purple" })[l] ||
  "gray";
const statusText = (s) =>
  ({ 1: "待处理", 2: "处理中", 3: "已解决", 4: "已关闭" })[s] || "未知";
const statusColor = (s) =>
  ({ 1: "red", 2: "orangered", 3: "green", 4: "gray" })[s] || "gray";

// 判断是否为严重故障 (level >= 3: HIGH, CRITICAL)
const isSevereFault = (level) => {
  return level === "HIGH" || level === "CRITICAL";
};

// 获取行样式类名
const getRowClass = (record) => {
  return isSevereFault(record.faultLevel) ? "severe-fault-row" : "";
};
...
async function submitHandle() {
  ...
  try {
    ...
    if (item) {
      ...
      if (handleForm.status === 3)
        item.resolveTime = new Date()
          .toISOString()
          .replace("T", " ")
          .substring(0, 19);
    }
...
function generateMockFaults() {
  ...
  for (let i = 1; i <= 36; i++) {
    ...
    data.push({
      id: i,
      faultCode: `FLT-2026-${String(i).padStart(4, "0")}`,
      turbineId: (i % 8) + 1,
      turbineCode: `WT-${String((i % 8) + 1).padStart(3, "0")}`,
      turbineName: `${(i % 8) + 1}号风机`,
      faultType: types[i % types.length],
      faultLevel: levels[i % levels.length],
      ...
      solution:
        status >= 3
          ? `已更换受损部件并完成调试，设备恢复正常运行。维修耗时约${2 + (i % 5)}小时。`
          : null,
      status,
      handler: status >= 2 ? "张工" : null,
      faultTime: `2026-02-${String(22 - (i % 20)).padStart(2, "0")} ${String(8 + (i % 12)).padStart(2, "0")}:${String(i % 60).padStart(2, "0")}:00`,
      resolveTime:
        status >= 3
          ? `2026-02-${String(22 - (i % 20)).padStart(2, "0")} ${String(12 + (i % 8)).padStart(2, "0")}:${String(i % 60).padStart(2, "0")}:00`
          : null,
    });
  }
  return data;
...
async function fetchList() {
  ...
  try {
    const res = await getFaultPage({
      ...
      faultType: search.faultType,
      faultLevel: search.faultLevel,
    ...
    if (search.faultType)
      filtered = filtered.filter((f) => f.faultType === search.faultType);
    if (search.faultLevel)
      filtered = filtered.filter((f) => f.faultLevel === search.faultLevel);
    if (search.status)
      filtered = filtered.filter((f) => f.status === search.status);
...
function initTypeChart(dist) {
  ...
  typeChartOption.value = {
    ...
    xAxis: {
      ...
      splitLine: { lineStyle: { color: darkAxis, type: "dashed" } },
    },
    yAxis: {
      type: "category",
      data: types,
...
function initLevelChart(dist) {
  ...
  const data = Object.entries(dist).map(([k, v]) => ({
    name: names[k] || k,
    value: v,
    itemStyle: { color: colors[k] || "#86909C" },
  }));
